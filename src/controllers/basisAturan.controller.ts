...
export const getAllBasisAturan = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    return successResponse<any[]>(res, 200, 'Success get all basis aturan', paginatedData, paginationMeta)
  } catch (error: any) {
    return errorResponse(res, 500, 'Failed get all basis aturan', error.message)
  }
}
...
export const createBasisAturanWithCertaintyFactor = async (req: Request, res: Response): Promise<any> => {
  ...
  if (existingRule) {
    ...
    return res.status(409).send({
      success: false,
      statusCode: 409,
      message: `Basis Pengetahuan untuk Karir ${karirName?.name} - ${karirName?.id} sudah dibuat`,
      data: null
    })
  }
