...
export const requireRoles = (allowedRoles: string[]) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    const user = res.locals.user // mengambil informasi dari header request token

    if (!user) {
      errorResponse(res, 401, 'Token tidak valid || Anda harus login terlebih dahulu', true)
      return
    }
    ...
    if (!allowedRoles.includes(user.role)) {
      errorResponse(res, 403, 'Role anda tidak punya akses ke endpoint ini', true)
      return
    }
